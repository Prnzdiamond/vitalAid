<template>
  <div class="min-h-screen flex flex-col bg-gray-100">
    <!-- Dashboard Header -->
    <header class="p-4 bg-white text-black flex justify-between items-center shadow-md">
      <h1 class="text-2xl font-bold">VitalAid Dashboard</h1>
      <div class="flex items-center gap-2">
        <span class="text-sm text-gray-600">{{ user._tag }}</span>
      </div>
    </header>
    ...
    <button @click="toggleChat" 
      class="fixed bottom-6 right-6 bg-green-600 text-white p-4 rounded-full shadow-lg hover:bg-green-700 transition">
    ...
    <Teleport to="body">
      <div v-if="showChat" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white w-80 rounded-lg shadow-lg p-4 relative">
          <button @click="toggleChat" class="absolute top-2 right-2 text-gray-500 hover:text-red-500">
...
// Define page as requiring authentication
...
const isDevelopment = process.env.NODE_ENV === 'development' || true; // For demo purposes
...
const handleRoleChange = async () => {
  // For demo purposes only - simulate different user roles
